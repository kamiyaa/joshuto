...
// use crate::run::AppBookmarkMapping;
...
pub struct TuiBookmarkMenu{ bookmarks:  AppBookmarkMapping, }

// TODO    lifetimes
impl TuiBookmarkMenu {
    pub fn new(context: &mut AppContext) -> Self {
        let bookmarks = context.bookmarks.clone();
        notify("!!!!!!!!!!!!!!!");
        notify(&bookmarks);
        Self {bookmarks,}
    }
    ...
    pub fn get_any_char<'a>(
        ...
        let _ = terminal.draw(|frame| {
...

            {
                // draw menu
                // let mut display_vec: Vec<String> = context.bookmarks.map
                let mut display_vec: Vec<String> = vec!["<any>".to_string()]; 
                // display_vec.sort();
                let display_str: Vec<&str> = display_vec.iter().map(|v| v.as_str()).collect();
                let display_str_len = display_str.len();
